...
typedef struct NPPFmtStageContext {
    int stage_needed;
...
typedef struct NPPFmtContext {
    const AVClass *class;

    NPPFmtStageContext stages[STAGE_NB];
    AVFrame *tmp_frame;
    int passthrough;
...
static int NPPFmt_init(AVFilterContext *ctx)
{
    NPPFmtContext *s = ctx->priv;
    int i;
...
static void NPPFmt_uninit(AVFilterContext *ctx)
{
    NPPFmtContext *s = ctx->priv;
    int i;
...
static int NPPFmt_query_formats(AVFilterContext *ctx)
{
...
static int init_stage(NPPFmtStageContext *stage, AVBufferRef *device_ctx)
{
...
static int NPPFmt_config_props(AVFilterLink *outlink)
{
...
static int NPPFmt_deinterleave(AVFilterContext *ctx, NPPFmtStageContext *stage,
...
static int NPPFmt_resize(AVFilterContext *ctx, NPPFmtStageContext *stage,
                           AVFrame *out, AVFrame *in)
{
    NPPFmtContext *s = ctx->priv;
    NppStatus err;
...
static int NPPFmt_interleave(AVFilterContext *ctx, NPPFmtStageContext *stage,
                               AVFrame *out, AVFrame *in)
{
...
static int (*const NPPFmt_process[])(AVFilterContext *ctx, NPPFmtStageContext *stage,
                                       AVFrame *out, AVFrame *in) = {
    [STAGE_DEINTERLEAVE] = NPPFmt_deinterleave,
    [STAGE_RESIZE]       = NPPFmt_resize,
    [STAGE_INTERLEAVE]   = NPPFmt_interleave,
};

static int NPPFmt_scale(AVFilterContext *ctx, AVFrame *out, AVFrame *in)
{
    NPPFmtContext *s = ctx->priv;
    AVFrame *src = in;
    int i, ret, last_stage = -1;
    ...
    for (i = 0; i < FF_ARRAY_ELEMS(s->stages); i++) {
        ...
        ret = NPPFmt_process[i](ctx, &s->stages[i], s->stages[i].frame, src);
        if (ret < 0)
            return ret;
...
static int NPPFmt_filter_frame(AVFilterLink *link, AVFrame *in)
{
...
#define OFFSET(x) offsetof(NPPFmtContext, x)
...
static const AVClass NPPFmt_class = {
    .class_name = "NPPFmt",
...
static const AVFilterPad NPPFmt_inputs[] = {
    {
        .name        = "default",
        .type        = AVMEDIA_TYPE_VIDEO,
        .filter_frame = NPPFmt_filter_frame,
    },
    { NULL }
};

static const AVFilterPad NPPFmt_outputs[] = {
    {
        .name         = "default",
        .type         = AVMEDIA_TYPE_VIDEO,
        .config_props = NPPFmt_config_props,
    },
    { NULL }
};

AVFilter ff_vf_fmt_npp = {
    .name      = "fmt_npp",
    .description = NULL_IF_CONFIG_SMALL("NVIDIA Performance Primitives video "
                                        "scaling and format conversion"),

    .init          = NPPFmt_init,
    .uninit        = NPPFmt_uninit,
    .query_formats = NPPFmt_query_formats,

    .priv_size = sizeof(NPPFmtContext),
    .priv_class = &NPPFmt_class,

    .inputs    = NPPFmt_inputs,
    .outputs   = NPPFmt_outputs,
